 ...
 *
 * Scope: this adapter only carries the reads EVHardware does not abstract — the
 ...
 * EVHardware instead, so they work across every supported generation.
 */
public class CarPropertyAdapter {
    ...
    // (SWI68/69/131/132/133/165) and the sibling EVProfile project branches its vehicle
    ...
    public void connect(Context context) {
        ...
        // EVHardware firmware detection to warn when running on another generation, where
        // these reads may return wrong or empty values (this app does not branch per gen).
        com.evsuite.hardware.FirmwareInfo firmwareInfo = com.evsuite.hardware.FirmwareInfo.INSTANCE;
        com.evsuite.hardware.FirmwareInfo.Gen gen = firmwareInfo.getGeneration();
        if (gen != com.evsuite.hardware.FirmwareInfo.Gen.SWI68) {
